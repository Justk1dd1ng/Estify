...
from crawlers.abstract import FlatPreview, Flat, FlatImage
from crawlers.rieltor_ua.locators import URL_GLOBAL
from crawlers.rieltor_ua.locators.flat_locators import RieltorFlatPreviewLocators, RieltorFlatLocators, \
    RieltorPremiumFlatLocators
from crawlers.rieltor_ua.pages.images import RieltorImageCrawler
...
class RieltorFlat(Flat):
    ...
    def get_imgs_list(self) -> List[FlatImage]:
        ...
        return [FlatImage(img_bin, self.id, img_id) for img_id, img_bin
                in enumerate(self.image_crawler.get_images_binary())]
        ...
        for param in self._get_params():
            if 'площадь' in param.select_one(self.locators.PARAM_TEXT).text.lower():
                try:
                    ...
                    return int(float(param.select_one(self.locators.PARAM_IMG).text.replace('м2', '')) + 0.5)
